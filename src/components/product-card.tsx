...
export function ProductCard({ product, className }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="group h-full" onClick={handleGuestClick}>
      <Card className={cn("overflow-hidden h-full flex flex-col transition-all duration-200 border border-foreground hover:shadow-primary/20 hover:shadow-lg hover:border-primary/50", className)}>
