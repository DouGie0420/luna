...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  // Default safety: disable SW in local dev to prevent stale chunk/cache issues.
  const allowInDev = process.env.NEXT_PUBLIC_ENABLE_PWA_DEV === 'true';
  if (process.env.NODE_ENV === 'development' && !allowInDev) {
    console.log('[PWA] Skip service worker registration in development');
    return null;
  }
