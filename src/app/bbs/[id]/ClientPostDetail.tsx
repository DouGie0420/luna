...
const CommentItem = ({ comment, user, canInteract, handleLikeDislike, setReplyingTo, handleInteractionNotAllowed, setCommentToDelete }: any) => {
    ...
    // 🔥 修复点 1：用 useMemo 缓存查询引用，防止子组件渲染死循环
    const authorRef = useMemo(() => firestore ? doc(firestore, 'users', comment.authorId) : null, [firestore, comment.authorId]);
...
export default function BbsPostPage() {
    ...
    // 🔥 修复点 2：为页面顶部的作者引用添加 useMemo
    const postAuthorRef = useMemo(() => firestore && post?.authorId ? doc(firestore, 'users', post.authorId) : null, [firestore, post?.authorId]);
    const { data: authorProfile } = useDoc<UserProfile>(postAuthorRef);
    ...
    // 🔥 修复点 3：拆分 Effect！单独处理浏览量增加，绝对不依赖任何 post 内部属性
    useEffect(() => {
        if (firestore && id) {
            updateDoc(doc(firestore, 'bbs', id), { views: increment(1) }).catch(() => {});
        }
    }, [firestore, id]); // <-- 剥离了 post?.videos，再也不会因为视图刷新而陷入死循环！

    // 🔥 修复点 4：提取布尔值作为依赖，单独处理 TikTok 脚本加载
    const hasTiktok = post?.videos?.some(url => url.includes('tiktok.com')) || false;
    useEffect(() => {
        if (hasTiktok) {
