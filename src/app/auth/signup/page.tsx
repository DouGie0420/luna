...
export default function SignUpPage() {
  ...
  const createUserProfile = async (userId: string, email: string, displayName: string) => {
    ...
    const pixelAvatarUrl = generatePixelAvatar(pixelAvatarSeed);
  ...
  const handleTwitterSignUp = async () => {
    ...
    try {
      ...
      toast({
        title: 'Welcome to LUNA!',
        description: 'Your account has been created with Twitter.',
      });
      router.push('/');
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-purple-900/20 to-black flex items-center justify-center p-4">
      <Card className="w-full max-w-md glass-morphism border-white/10 p-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gradient mb-2">Join LUNA</h1>
          <p className="text-white/60">Create your account and start trading</p>
        </div>
        ...
        <form onSubmit={handleEmailSignUp} className="space-y-4">
          <div>
            <label className="text-sm text-white/70 mb-2 block">Display Name</label>
            <Input
              type="text"
              value={displayName}
