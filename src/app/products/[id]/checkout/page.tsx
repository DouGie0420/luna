...
import { ethers, formatUnits, parseUnits, BigNumberish } from 'ethers'; // formatUnits 和 parseUnits 仍需要
...
import { Edit, Loader2, MapPin, ShieldCheck, AlertCircle, ShoppingCart } from 'lucide-react'; // 移除旧的Web3图标
...
import { Input } from '@/components/ui/input'; // 确保 Input 已导入
...
export default function CheckoutPage() {
  ...
  const [isApproveDialogOpen, setIsApproveDialogOpen] = useState(false); // USDT 授权对话框
  const [amountToApprove, setAmountToApprove] = useState<string>(''); // 授权金额输入
  ...
  useEffect(() => {
    if (progress < 90 && isProcessing) { // 只有在处理中才更新进度条
      const timer = setTimeout(() => setProgress(p => p + 1), 50);
      return () => clearTimeout(timer);
    }
    if (!isProcessing && progress > 0 && progress < 100) { // 如果处理结束但进度未到100，重置
        setProgress(0);
    }
  ...
  useEffect(() => {
    if (isConnected && chainId !== REQUIRED_CHAIN_ID) {
        toast({
            ...
            description: `请切换到 Polygon 主网 (Chain ID: ${REQUIRED_CHAIN_ID}) 进行交易。`,
            action: (
                <Button
  ...
  const totalAmount = useMemo(() => (product?.price || 0) + shippingFee, [product, shippingFee]); // 法币总额
  ...
  const handleConfirmPurchase = async () => {
    ...
    if (!product.escrowOrderId) {
        ...
        // 这通常意味着后端需要在商品发布时调用 createOrder
        return;
    }
    ...
    if (isUSDTDataLoading || usdtDataError || usdtBalance === null || usdtAllowance === null || usdtDecimals === null) {
        ...
        refetchUSDTData(); // 尝试刷新数据
        return;
    }

    const amountInUnits = parseUnits(totalAmount.toString(), usdtDecimals); // 将法币总额转换为 USDT 最小单位
    ...
    if (usdtAllowance < amountInUnits) {
        ...
        setIsApproveDialogOpen(true); // 弹出授权对话框
        return;
    }
    // --- End USDT 相关检查 ---
    ...
    try {
      ...
      const { id: addrId, isDefault, ...shippingAddress } = address; // 避免Firebase存储ID和isDefault
      ...
      const orderData = {
        ...
        totalAmount,
        currency: 'USDT',
        status: 'paid', // 已通过链上校验，状态设为已支付
        escrowOrderId: product.escrowOrderId, // 存储合约订单ID
        txHash: lockResult.hash || 'N/A', // 使用实际的交易哈希
