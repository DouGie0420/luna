    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (foundUser) {
                setUser(foundUser);
                // Mock followers data
                const mockFollowers = allUsers.filter(u => u.id !== userId).slice(0, Math.floor(Math.random() * 8) + 2);
                setFollowers(mockFollowers);
                const initialStatus: Record<string, boolean> = {};
                mockFollowers.forEach(u => {
    ...
    <>
      ...
      <div className="container mx-auto px-4 py-12 max-w-2xl">
        <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-3xl font-headline">{t('userProfile.usersFollowers').replace('{userName}', user.name)}</h1>
        </div>
            ...
            <CardContent className="p-0">
                ...
                <div className="divide-y">
                    ...
                    <div key={follower.id} className="flex items-center gap-4 p-4">
                        <Link href={`/user/${follower.id}`} className="flex items-center gap-4 flex-1">
                ...
                <div className="p-12 text-center text-muted-foreground">
                    <p>{t('userProfile.noFollowers').replace('{userName}', user.name)}</p>
                </div>
                )}
            </CardContent>
        </Card>
