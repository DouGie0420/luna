        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (foundUser.following && foundUser.following.length > 0) {
                    ...
                    const followingQuery = query(usersRef, where('uid', 'in', foundUser.following.slice(0, 30)));
                    const followingSnapshot = await getDocs(followingQuery);
                    setFollowing(followingSnapshot.docs.map(d => d.data() as UserProfile));
                } else {
                    setFollowing([]);
                }
            } catch (e) {
                        ...
                        {following.length > 0 ? (
                            <div className="divide-y">
                                {following.map((followedUser) => (
                                    <div key={followedUser.uid} className="flex items-center gap-4 p-4 hover:bg-accent transition-colors">
                                        <Link href={followedUser.loginId ? `/@${followedUser.loginId}` : `/user/${followedUser.uid}`} className="flex items-center gap-4 flex-1">
                                            ...
                                            <p className="font-headline font-semibold hover:underline">{followedUser.displayName}</p>
                                        </Link>
                                        {currentUser?.uid !== followedUser.uid && (
                                            <Button
